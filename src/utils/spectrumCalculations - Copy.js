...
function parseRangeToMHz(rangeStr) {
  ...
  return {
    startMHz: parsePart(parts[0]),
    endMHz: parsePart(parts[1]),
  };
}

// Categorise utilization
...
export function computeLocationStats(locationObj) {
  ...
  return { avgUtilization, categorySizesMHz, bandAverages };
}

// NATIONAL STATS (supports regions + normal locations)
export function computeNationalStats() {
  ...
  BandsData.forEach((loc) => {
    const regions = Array.isArray(loc.regions)
      ? loc.regions
      : [{ bands: loc.bands || [] }];
