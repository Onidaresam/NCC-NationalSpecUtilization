// src/components/LocationDashboard.js

import React from "react";
import { Doughnut } from "react-chartjs-2";
import BandsData from "../Data/Bands/data";
import { computeLocationStats } from "../utils/spectrumCalculations";

export default function LocationDashboard({ locationName }) {
  ...
  const { avgUtilization, categorySizesMHz, bandAverages } =
    computeLocationStats(locationObj);
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-8">
      {/* LEFT: Location average + categories */}
      <div className="flex-1 bg-white/10 backdrop-blur-md rounded-xl shadow-md border border-white/20 p-6">
        <h2 className="text-xl font-bold text-white mb-4">
          {locationName} Spectrum Utilization (100 kHz – 6 GHz)
        </h2>
