...
export default function LocationDashboard({ locationName }) {
  const locationObj = computeLocationStats(locationName);

  if (!locationObj) {
    return (
      <div className="text-center text-[#4F00B6] font-bold text-xl">
        No data available for {locationName}
      </div>
    );
  }

  const { avgUtilization, categorySizesMHz, bandAverages } = locationObj;
    ...
    <div className="w-full flex flex-col lg:flex-row gap-8 animate-fadeIn">
      ...
      <div className="flex-1 bg-white rounded-xl shadow-xl border border-gray-200 p-6">
        <h2 className="text-2xl font-bold text-[#4F00B6] mb-4">
          {locationName} — Spectrum Utilization Overview
        </h2>
